...
pub fn get_pkg_info(line: String) -> (String, String, String) {
    ...
    let output = if distro.contains("Red Hat") == true {
        ...
        //get name of package
        let package_name = if package_data[0] == "Installed" {
            "Box2D.x86_64"
        } else {
            package_data[0]
        };
    ...
    println!(
        "====== Package: {} ======",
        package_name.truecolor(135, 206, 250)
    );
    println!("{}: {} ", "Description".color("green"), pkg_descript.trim());

    let (build_date, diff) = calculate_dates(output, "Build Date", "Build Host");
    println!(
        "{}{} ==> {} days since upstream dnf repo update",
        "Last build date: ".truecolor(183, 150, 14),
        build_date,
        diff.num_days().to_string().truecolor(219, 78, 17)
    );

    let (install_date, install_diff) = calculate_dates(
        String::from_utf8_lossy(&execution_str.stdout).to_string(),
        "Install Date:",
        "Group",
    );
    println!(
        "{}{} ==> {} days since you updated",
        "Last install date: ".truecolor(255, 127, 80),
        install_date,
        install_diff.num_days().to_string().truecolor(219, 78, 17)
    );
    ...
    let output = String::from_utf8_lossy(&execution_str.stdout).to_string();
    let start_ver = output.find("Version     :").unwrap_or(0);
    let end_ver = output.find("Release").unwrap_or(output.len());
    let pkg_version = &mut output[start_ver + 14..end_ver].trim().to_string();
